...
def custom_admin_dashboard_view(request):
    ...
    admin_actions = ModeratorAction.objects.filter(
        moderator__is_staff=True
...
def custom_admin_pda_list_view(request):
    ...
    if filter_type == 'pending':
        submissions = PublicDeepfakeArchive.objects.filter(
            is_approved=False, 
            review_date__isnull=True
...
def custom_admin_logs_view(request):
    ...
    context = {
        ...
        'moderators': User.objects.filter(
            Q(is_staff=True) | Q(groups__name='PDA_Moderator')
        ).distinct()
    }
...
def custom_admin_moderators_view(request):
    ...
    moderator_group = Group.objects.get(name='PDA_Moderator')
    moderators = User.objects.filter(
        Q(groups=moderator_group) | Q(is_staff=True)
    ...
    regular_users = User.objects.exclude(
        Q(groups=moderator_group) | Q(is_staff=True) | Q(is_superuser=True)
    ).order_by('username')
    
    context = {
        'active_page': 'moderators',
...
def custom_admin_pending_view(request):
    ...
    pending_pda = PublicDeepfakeArchive.objects.filter(
        is_approved=False, 
        review_date__isnull=True
    ).order_by('-submission_date')
    
    pending_threads = ForumThread.objects.filter(
        approval_status='pending'
    ).order_by('-created_at')
